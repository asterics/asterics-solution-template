...
/**
 * Sets the areBaseURI either with a default value for the protocol of the hosting webserver (http, https) or with the value given by a queryParameter.
 */
function initAreBaseURI() {
  //Set the base URI of the running ARE instance.

  //Use default value, depending on the protocol we are hosted on.
  ...
  if (urlParams.has("areBaseURI") === true) {
    areBaseURI = urlParams.get("areBaseURI") + "/rest/";
  }
  console.log("Setting areBaseURI: " + areBaseURI);
  setBaseURI(areBaseURI);
}
...
function loadFileFromWebServer(relFilePath, successCallback, errorCallback) {
  ...
  var pathFix = "";
  if (
    Object.is(location.origin, "https://asterics.github.io") ||
    Object.is(location.origin, "http://asterics.github.io")
  ) {
    pathFix = "/AsTeRICS";
  ...
  httpReq.onreadystatechange = function() {
    if (
      ...
      (httpReq.status === 404 || httpReq.status === 0)
    ) {
 ...
 * @param {function(HTTPstatus, errorMessage)} [errorCallback=defaultErrorCallback]- Callback function in case of an error. 
 */
function deployModelFromWebserverApplySettingsAndStartModel(
  relFilePath,
  propertyMap,
  successCallback,
  errorCallback
 ...
 * @param {function(HTTPstatus, errorMessage)} [errorCallback=defaultErrorCallback]- Callback function in case of an error.
 */
function storeFileFromWebserverOnARE(
  relFilePath,
  relFilePathARE,
  successCallback,
  errorCallback
...
function saveSettingsAsAutostartModel(
  modelFilePathOnWebserver,
  successCallback,
  errorCallback
) {
...
function updateModelPropertiesFromWidgets(modelInXML) {
  ...
  console.log(
    "Updating " +
      widgetIdToPropertyKeyMap.length +
      " widgets from model properties."
  );
  ...
  for (var i = 0; i < widgetIdToPropertyKeyMap.length; i++) {
    ...
    if (typeof widgetVal != "undefined") {
      ...
      console.log(
        "widgetId <" + widgetIdToPropertyKeyMap[i]["widgetId"] + "=undefined>"
      );
    }
  }
...
function updateWidgetsFromModelProperties(modelInXML) {
  ...
  for (var i = 0; i < widgetIdToPropertyKeyMap.length; i++) {
    ...
    if (typeof propVal != "undefined") {
      console.log(
        "Updating widget <" +
          widgetIdToPropertyKeyMap[i]["widgetId"] +
          "=" +
          propVal +
          ">"
      );
